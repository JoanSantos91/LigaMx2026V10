...
def player_view(user):
    ...
    with tabs[0]:
        ...
        if st.button("Enviar pronósticos y Survivor",type="primary",disabled=locked,use_container_width=True):
            if not survivor_choice:
                ...
                try:
                    with conn() as c:
                        ...
                        c.execute("""INSERT INTO survivor_picks(user_id,round_id,team,submitted_at) VALUES(?,?,?,?)
                                     ON CONFLICT(user_id,round_id) DO UPDATE SET team=excluded.team,submitted_at=excluded.submitted_at""",
                                  (user["id"],round_row["id"],survivor_choice,now_local().isoformat()))
                    st.success("Jornada enviada: pronósticos y Survivor quedaron guardados.")
                except sqlite3.IntegrityError:
    ...
    with tabs[2]:
        st.info("La elección Survivor se realiza obligatoriamente al final de la pestaña Pronósticos y se envía junto con toda la jornada.")
    ...
    with tabs[4]:
        ...
        journey=st.selectbox("Detalle de jornada",range(1,18)); render_pro_table(pd.DataFrame(duels_round(journey)), f"Duelos · Jornada {journey}", rank_col="", team_by_player=False)
...
def admin_view():
    ...
    with tabs[2]:
        ...
        status,complete,override=round_submission_status(round_row["id"])
        ready=sum(1 for row in status if row["ESTADO"]=="Listo")
        visible = complete or override
        a,b=st.columns(2); a.metric("Entregaron",f"{ready}/18"); b.metric("Publicación", "Visible" if visible else "Bloqueada")
        if not complete:
            button_label = "Revocar publicación anticipada" if override else "Autorizar publicación aunque falten jugadores"
            button_type = "secondary" if override else "primary"
            if st.button(button_label, type=button_type, use_container_width=True):
                with conn() as c:
                    c.execute("UPDATE rounds SET reveal_override=? WHERE id=?", (0 if override else 1, round_row["id"]))
                st.rerun()
            if override:
                st.warning("La publicación anticipada está activa. Los pronósticos ya son visibles aunque haya participantes pendientes.")
        elif override:
            # Cuando todos completan, la autorización ya no es necesaria; se limpia automáticamente.
            with conn() as c:
                c.execute("UPDATE rounds SET reveal_override=0 WHERE id=?", (round_row["id"],))
    ...
    with tabs[5]:
        render_pro_table(duel_standings(), "Tabla general de duelos")
        journey=st.selectbox("Jornada de duelos",range(1,18)); render_pro_table(pd.DataFrame(duels_round(journey)), f"Duelos · Jornada {journey}", rank_col="", team_by_player=False)
    with tabs[6]: render_pro_table(survivor_status(), "Tabla Survivor")
    with tabs[7]:
